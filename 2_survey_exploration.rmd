...
#across means mutate multiple columns not just one, then you have to specify. So only when its numeric. Lamda needed the moment that you are putting arguments inside a function (~round(.x, digits = 0)(arguments would not be needed if rounded to 0).
...
```{r}
survey_mean <-survey_round %>%
  select(where(is.numeric)) %>% 
  map_df( ~mean(.x, na.rm = TRUE)) %>% 
  kable(col.names = c("Span of writing hand (cm)" , "Span of non-writing hand (cm)", "Pulse Rate(bpm)", "Height (cm)", "Age (years)"),
        caption = "Means for survey",
    align = "ccccc"
  ) 

  
#map_df is the tidyverse version of a loop. 2 arguments 1st tell which columns, then the function you want to use.
...
survey_round_mean <- survey_round %>% 
  ...
  pivot_wider(names_from = name,
              values_from = mean) %>% 
  kable(col.names = c("Age (years)", "Height (cm)", "Span of non-writing hand (cm)","Pulse Rate(bpm)" , "Span of writing hand (cm)"),
        digits = 0,
        caption = "Means for survey",
    align = "ccccc"
  ) 
...
```{r}
new <- survey %>% 
  mutate(Height_original = case_when(M.I == "Imperial" ~ Height*0.393701,
                                     M.I == "Metric" ~ Height) )
